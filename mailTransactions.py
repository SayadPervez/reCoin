...
def RUN(arg):
    ...
    while(i<30 and i<len(rows)):
        if(arg in rows[i]):
            l.append(rows[i])
        i+=1
    d="If your transaction data is not visible, Click 'Show quoted text' to view your Transactions data:<br><br><h2>Your transaction data:</h2><div style='border:3px solid red;border-radius:6px;padding:2px 2px;margin:1px auto;border-top:0px;border-bottom:0px;'>"
    for _ in l:
