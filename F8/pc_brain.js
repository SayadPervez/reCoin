...
function totp()
{
    ...
    amt=Number(document.getElementById("tAmt").value);
    console.log(amt,myCoins);
    if(amt<=myCoins)
    {
        document.getElementById("tsp").innerHTML="Please wait...";
        document.getElementById("tsp").style.display="block";
    reply=shell(hyb("reqTransOTP",{sender:myUserName,receiver:rec_ver,amount:amt}));
    if(reply.status=="success"){
        document.getElementById("tsp").innerHTML="Sending OTP<br>Please wait...";
        document.getElementById("tsp").style.display="block";
        page2(decode(reply.reply),decode(reply.code));
    }
    else{
        document.getElementById("tsp").innerHTML="Sorry Some Error Occured";
        document.getElementById("tsp").style.display="block";
    }}
    else{
        document.getElementById("tsp").innerHTML="Sorry, You don't have enough Balance";
        document.getElementById("tsp").style.display="block";
    }
}

function resendTOTP(){
    document.getElementById("rstotp").innerHTML="Plesae wait...";
    reply=shell(hyb("reqTransOTP",{sender:myUserName,receiver:rec_ver,amount:amt}));
    if(reply.status=="success")
    {
        ...
        document.getElementById("rstotp").innerHTML="OTP Resend Failed";
    }
}

function submitTOTP(){
    var w=document.getElementById("o_t_p").value;
    reply=shell(hyb("confirmTransfer",{uname:myUserName,code_:encode(String(w))}));
    if(reply.status=="success")
        document.getElementById("rstotp").innerHTML="Transaction Successful !!";
    else
    document.getElementById("rstotp").innerHTML="Transaction Failed !!";
}
...
function T(){
    ...
    if(rply.status!="success")
        ...
        myCoins=Number(decode(rply.code));
        console.log("mycoins",decode(rply.code));
        page2(decode(rply.reply),"none");
    }
}
